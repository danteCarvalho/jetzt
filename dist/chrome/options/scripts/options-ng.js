...
optsApp.controller("AppearanceCtrl", function ($scope, jetzt, Persist) {
	...
	if (window.chrome && window.chrome.fontSettings && typeof window.chrome.fontSettings.getFontList === "function") {
		...
		// Fallback for Firefox or browsers where fontSettings is not available
		var standardFonts = ["Arial", "Courier New", "Georgia", "Helvetica", "Impact", "Times New Roman", "Trebuchet MS", "Verdana", "Menlo", "Monaco", "Consolas"];
		$scope.installedFonts = standardFonts.map(function(f) {
			return { fontId: f, displayName: f };
		});
	}
